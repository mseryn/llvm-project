...
// TODO move into header file when done
typedef struct ring_buffer_t ring_buffer_t;
typedef ring_buffer_t* ring_buffer_handle_t;
ring_buffer_handle_t ring_buffer_init(uint8_t* buffer, size_t size);
void ring_buffer_free(ring_buffer_handle_t ring_buffer);
bool ring_buffer_full(ring_buffer_handle_t ring_buffer);
size_t ring_buffer_max_size(ring_buffer_handle_t ring_buffer);
void ring_buffer_push(ring_buffer_handle_t ring_buffer, uint8_t data);
void ring_buffer_pop(ring_buffer_handle_t ring_buffer, uint8_t data);
...
ring_buffer_handle_t ring_buffer_init(uint8_t* buffer, size_t max_size) {
  ring_buffer_handle_t ringbuf = malloc(sizeof(ring_buffer_t));
  ring_buffer->buffer = buffer;
  ring_buffer->max_size = max_size;
  ring_buffer->head = 0;
  ring_buffer->tail = 0;
...
bool ring_buffer_empty(ring_buffer_handle_t ring_buffer) {
  return (!ring_buffer->is_full && (ring_buffer->head == ring_buffer->tail));
}

size_t ring_buffer_max_size(ring_buffer_handle_t ring_buffer) {
  return ring_buffer->max_size;
}
...
void ring_buffer_push(ring_buffer_handle_t ring_buffer, uint8_t data) {
  ...
  if (ring_buffer->is_full) {
    if (++(ring_buffer->tail) == ring_buffer->max_size) {
      ring_buffer->tail = 0;
    }
  }
  // Also wrap the head if applicable
  if (++(ring_buffer->head) == ring_buffer->max_size) {
    ring_buffer->head = 0;
  }

  // Recalculate full status of buffer
  ring_buffer->is_full = (ring_buffer->head == ring_buffer->tail);
}

int ring_buffer_pop(ring_buffer_handle_t ring_buffer, uint8_t *data) {
  // Returns -1 if buffer empty, TODO how best to handle this?
  int ret = -1;
  else {
    ...
    *data = ring_buffer->buffer[ring_buffer->tail];
    ...
    // Move tail forward and wrap if applicable
    if (++(ring_buffer->tail) == ring_buffer->max_size) {
      ring_buffer->tail = 0;
    }
